...
export class ChannelService {
	...
	async getAllChannelConnections(pagination: PaginationDto): Promise<ChannelConnection[]> {
	...
	async getChannelConnections(channelId: number, pagination: PaginationDto): Promise<ChannelConnection[]> {
	...
	async join(user: User, id: number, data: ChannelDto): Promise<ChannelConnection> {
		...
		if (!channelConnection) return await this.joinChannel(user, channel, data.password, data.socketId);
		...
		if (channelConnection && data.socketId) this.chatService.joinRoom(data.socketId, channel.id.toString());
	...
	async updateChannelRole(role: ChannelRole, channelConnection: ChannelConnection) {
	...
	async joinChannel(user: User, channel: Channel, password?: string, socketId?: string): Promise<ChannelConnection> {
