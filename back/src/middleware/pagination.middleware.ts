...
export class PaginationMiddleware implements NestMiddleware {
	async use(req: Request, res: Response, next: NextFunction) {
		const pagination: PaginationDto = plainToInstance(PaginationDto, req.query, { enableImplicitConversion: true });
		...
		if (errors.length > 0) {
			 ...
			 */
			const constraints: string[] = errors.reduce((accumulator: string[], error: ValidationError) => {
